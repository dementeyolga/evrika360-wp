(function () {
  ...
  if (deptSliderViewportNode && deptSliderOptions) {
    const deptSliderEmblaApi = EmblaCarousel(
      deptSliderViewportNode,
      deptSliderOptions,
    );

    const deptSliderAddTogglePrevNextBtnsActive = (
      emblaApi,
      prevBtn,
      nextBtn,
    ) => {
    ...
    const deptSliderAddPrevNextBtnsClickHandlers = (
      emblaApi,
      prevBtn,
      nextBtn,
    ) => {
      if (emblaApi && prevBtn && nextBtn) {
        ...
        const removeTogglePrevNextBtnsActive =
          deptSliderAddTogglePrevNextBtnsActive(emblaApi, prevBtn, nextBtn);
    ...
    const deptSliderRemoveDotBtnsAndClickHandlers =
      deptSliderAddDotBtnsAndClickHandlers(
        deptSliderEmblaApi,
        deptSliderDotsNode,
      );

    deptSliderEmblaApi?.on(
      "destroy",
      deptSliderRemovePrevNextBtnsClickHandlers,
    );
