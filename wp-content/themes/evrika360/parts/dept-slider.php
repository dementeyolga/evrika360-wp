<section class="embla dept-embla">
  <div class="embla__viewport dept-embla__viewport">
    <div class="embla__container">
      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-owner.png" alt="владелец">
          </div>
          <div class="content">
            <h3>Владельцу</h3>
            <ul>
              <li class="flash-li">Сокращение расходов</li>
              <li class="flash-li">Уведомления о критических ситуациях</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-sales.png" alt="отдел продаж">
          </div>
          <div class="content">
            <h3>Отделу продаж</h3>
            <ul>
              <li class="flash-li">Рост конверсий в продажу</li>
              <li class="flash-li">Контроль чек-листов</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-marketing.png" alt="оттдел маркетинга">
          </div>
          <div class="content">
            <h3>Отделу маркетинга</h3>
            <ul>
              <li class="flash-li">Автоматический анализ качества лидов</li>
              <li class="flash-li">Определение болей клиентов</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-qa.png" alt="отдел контроля качества">
          </div>
          <div class="content">
            <h3>Отделу контроля качества</h3>
            <ul>
              <li class="flash-li">100% прослушка звонков</li>
              <li class="flash-li">Мгновенное получение данных в любых разрезах</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-hr.png" alt="отдел hr">
          </div>
          <div class="content">
            <h3>Отделу HR</h3>
            <ul>
              <li class="flash-li">Предотвращение выгорания сотрудников</li>
              <li class="flash-li">Сокращение времени на подготовку итогов встречи</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="embla__slide">
        <div class="dept-slide">
          <div class="image-container">
            <img src="<?php echo get_template_directory_uri() ?>/assets/images/slider-security.png" alt="служба безопасности">
          </div>
          <div class="content">
            <h3>Службе безопасности</h3>
            <ul>
              <li class="flash-li">Контроль утечки данных</li>
              <li class="flash-li">Мгновенные уведомления о критических ситуациях</li>
            </ul>
          </div>
        </div>
      </div>

    </div>
  </div>

  <div class="embla__buttons">
    <button class="rotate-180 embla__button embla__button--prev dept-embla__button--prev" type="button">
      <?php include 'arrow-right.php' ?>
    </button>

    <button class="embla__button embla__button--next dept-embla__button--next" type="button">
      <?php include 'arrow-right.php' ?>
    </button>
  </div>

  <div class="embla__dots dept-embla__dots"></div>
</section>